...
class BLEService:
    ...
    def __init__(self):
        ...
        self._scan_cb = None
        self._gatt_cb = None  # Reference to Java GattCallback wrapper
        self._gatt_listener = None  # Reference to PyJNIus listener interface
    ...
    def prompt_enable_bluetooth(cls):
        ...
        try:
            ...
            print(
                "[BLEService] Triggered system Bluetooth request dialog."
            )
        except Exception as e:
    ...
    def scan(self, on_complete_callback):
        ...
        try:
            ...
            print("[BLEService] Legacy interface scan started safely...")
        except Exception as e:
    ...
    def connect(self, target, on_result=None):
        """Connects to a target BLE device using the org.kivy.android.GattCallback wrapper."""
        ...
        if not device:
            print(
                f"[BLEService] Could not resolve remote device {mac_address}."
            )
            if on_result:
                Clock.schedule_once(lambda dt: on_result(False), 0)
            return
        ...
        class GattListenerHelper(PythonJavaClass):
            ...
            def onConnectionStateChange(self, gatt, status, newState):
                if newState == BluetoothProfile.STATE_CONNECTED:
                    print(
                        f"[BLEService] Connected to GATT server on {mac_address}"
                    )
            ...
            def onServicesDiscovered(self, gatt, status):
                if status == 0:
                    print(
                        "[BLEService] GATT Services Discovered successfully."
                    )
                    try:
                        ...
                        for i in range(services.size()):
                            ...
                            for j in range(chars.size()):
                                c = chars.get(j)
                                char_uuid = (
                                    c.getUuid().toString().lower()
                                )
                                if (
                                    service_self.UART_RX_CHAR_UUID
                                    in char_uuid
                                ):
                                    service_self._write_characteristic = c
                                    print(
                                        f"[BLEService] Matched TX/RX Characteristic: {char_uuid}"
                                    )
                                    break
                    except Exception as err:
                        print(
                            f"[BLEService] Characteristic discovery error: {err}"
                        )
                else:
                    print(
                        f"[BLEService] Service discovery failed with status: {status}"
                    )

        self._gatt_listener = GattListenerHelper()
        self._gatt_cb = GattCallback(self._gatt_listener)

        print(f"[BLEService] Initiating GATT connection to {mac_address}...")
        try:
            self.gatt = device.connectGatt(activity, False, self._gatt_cb)
        except Exception as e:
            print(f"[BLEService] Exception during connectGatt: {e}")
            self.is_connected = False
            if on_result:
                Clock.schedule_once(lambda dt: on_result(False), 0)

    def send_command(self, data_bytes):
        """Writes control bytes to the connected robot."""
    ...
    def disconnect(self, on_complete=None):
        """Gracefully closes the GATT connection."""
        if self.gatt:
